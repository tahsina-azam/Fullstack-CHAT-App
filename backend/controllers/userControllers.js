...
export const signIn = asyncHandler(async(req,res) => {
    try {
        ...
        const token = jwt.sign({ email: req.body.email }, 'supersecret', { expiresIn: '2h' });
        // Set the token as a cookie in the response
        res.setHeader('Set-Cookie', `token=${token}; HttpOnly; Max-Age=${2 * 60 * 60}; Path=/`);

        return res.status(200).json({ message: "Logged in successfully", email: req.body.email, user_id: user._id, firstName:user.firstName, lastName:user.lastName });
    } catch (error) {
        console.log(error)
